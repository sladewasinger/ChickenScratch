...
export class CreateLobbyComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    this.form = new FormGroup({
      lobbyName: new FormControl(null, [Validators.required, Validators.minLength(3)])
    });
  }

  get isLobbyNameControlInvalid(): boolean {
    return this.lobbyNameControl?.invalid && (this.lobbyNameControl?.touched || this.lobbyNameControl?.dirty);
  }
  ...
  onSubmit(): void {
    ...
    this.hubSocketService.sendWithPromise<HubResponse<LobbyState>>("createLobby", {
      lobbyName: this.lobbyNameControl.value,
    }).then(response => {
      if (!response.isSuccess) {
        throw response;
      }
      const myLobby = response.data.lobbies.find(l => l.players.find(p => p.id == this.myPlayer!.id))
