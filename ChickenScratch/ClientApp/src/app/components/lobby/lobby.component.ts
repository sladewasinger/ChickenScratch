...
export class LobbyComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.subs.push(
      ...
      this.lobbyStateService.getLobbyState().subscribe((ls) => {
        this.lobby = ls?.lobbies.find((x) =>
          x.players.some((p) => p.id == this.myPlayer?.id)
        );
      })
    );
  ...
  playerNameEntered() {
    this.hubSocketService
      .sendWithPromise<HubResponse<LobbyState>>('createPlayer', {
  ...
  async sleep(sleepDurationMs: number) {
    if (sleepDurationMs < 0)
      throw new Error('sleepDurationMs must be positive');
  ...
  async joinLobby() {
    ...
    try {
      var response = await this.hubSocketService.sendWithPromise<
        HubResponse<LobbyState>
      >('joinLobby', {
  ...
  async startGame() {
    try {
      var result = await this.hubSocketService.sendWithPromise<any>(
        'StartGame',
        {}
      );
